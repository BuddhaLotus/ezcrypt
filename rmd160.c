...
\********************************************************************/
//#include <ntddk.h>
// modified by Stefan Scherrer to allow compilation with CrossCrypt Windows
// license pleas check netbsd and
...
RMD160Final(u_char digest[20], RMD160_CTX *context)
{
	...
	if (digest != NULL) {
		for (i = 0; i < 20; i += 4) {
			...
			digest[i]     =  (UCHAR)context->state[i>>2];
			digest[i + 1] = (UCHAR)(context->state[i>>2] >>  8);
			digest[i + 2] = (UCHAR)(context->state[i>>2] >> 16);
			digest[i + 3] = (UCHAR)(context->state[i>>2] >> 24);
		}
	}
}

void
RMD160Update(RMD160_CTX *context, u_char *data, u_int32_t nbytes)
{
	u_int32_t X[16];
	u_int32_t ofs = 0;
	u_int32_t i;
